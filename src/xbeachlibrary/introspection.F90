module introspection_module
   use iso_c_binding,    only: c_int, c_loc, c_ptr, c_double, c_f_pointer, c_char, C_NULL_CHAR
   use iso_c_utils,      only: MAXSTRINGLEN, char_array_to_string, string_to_char_array
   use mnemmodule,       only: arraytype, chartoindex
   use spaceparams,      only: indextos
   use libxbeach_module, only:par,s,tpar
...
contains
   ...
   integer(c_int) function getnparameter_fortran(n) bind(C, name="getnparameter")
      ...
      use getkey_module, only: getkeys
   ...
   integer(c_int) function getparametertype_fortran(name, typecode)
      use iso_c_utils, only: string_to_char_array
   ...
   integer(c_int) function getparametertype_c(name, typecode, length) bind(C,name="getparametertype")
      use getkey_module, only: getkey_indextype
   ...
   integer(c_int) function getarray(name, x, length) bind(C, name="getarray")
      ...
      use mnemiso_module, only:carraytype, arrayf2c
