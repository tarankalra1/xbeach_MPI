         ...
         do i=1,par%nmeanvar
            ...
            select case (t%rank)
             case (2)
               ...
               meansparsglobal(i)%min2d = -dFill
               meansparsglobal(i)%max2d = dFill
             case (3)
               if (t%type == 'r') then
#ifdef USEMPI
                  t%r3 = 0
               ...
               meansparsglobal(i)%min3d = -dFill
               meansparsglobal(i)%max3d = dFill
             case (4)
               if (t%type == 'r') then
#ifdef USEMPI
                  t%r4 = 0
               ...
               meansparsglobal(i)%min4d = -dFill
               meansparsglobal(i)%max4d = dFill
            end select
#ifdef USEMPI
            ...
            select case (t%rank)
             case (2)
               ...
               meansparslocal(i)%min2d = -dFill
               meansparslocal(i)%max2d = dFill
             case (3)
               if (t%type == 'r') then
                  d1 = size(t%r3,1)
                  d2 = size(t%r3,2)
               ...
               meansparslocal(i)%min3d = -dFill
               meansparslocal(i)%max3d = dFill
             case (4)
               if (t%type == 'r') then
                  d1 = size(t%r4,1)
                  d2 = size(t%r4,2)
               ...
               meansparslocal(i)%min4d = -dFill
               meansparslocal(i)%max4d = dFill
            end select
      ...
      do i=1,par%nmeanvar
         ...
         select case (t%rank)
          case (2)
            ...
            elseif(par%meanvars(i)=='zswet') then
               ! special case: only want to keep track of min/max if the point is wet
               ! cell if wet if zsdry>-huge(0.d0)
               where(tvar2d>dFill+1.d0)
                  meansparslocal(i)%mean2d = meansparslocal(i)%mean2d + mult*tvar2d
               endwhere
            else
               meansparslocal(i)%mean2d = meansparslocal(i)%mean2d + mult*tvar2d
            endif
            if(par%meanvars(i)=='zswet') then
               where(tvar2d>dFill+1.d0)
