...
#endif
   use xmpi_module
   use constants, only: iFill,sFill,dFill
#ifdef USENETCDF
   use netcdf
#endif
   use mnemmodule,    only: mnemonics, numvars
...
contains
   ...
   subroutine ncoutput_init(s, sl, par, tpar)
      ...
      use spaceparams,     only: indextos
      use spaceparamsdef
      use mnemmodule,      only: arraytype, chartoindex
      use timestep_module, only: timepars
      use means_module,    only: meanspars, meansparsglobal
      use logging_module,  only: writelog
      ! This module is awaiting comments from Robert.
      use getkey_module,   only: parameter, getkey, getkeys
      use paramsconst
            ...
            select case(t%type)
             case('r')
               ...
               do j = 1,nmeanvartypes
                  ...
                  if(trim(meanvartypes(j))=='min' .and. trim(t%name)=='zswet') then
                     ! special case
                     if(par%outputprecision == OUTPUTPRECISION_SINGLE) then
                        NF90(nf90_put_att(ncid, meanvarids(i,j), '_FillValue', -sFill))
                     else
                        NF90(nf90_put_att(ncid, meanvarids(i,j), '_FillValue', -dFill))
                     endif
                  else
                     if(par%outputprecision == OUTPUTPRECISION_SINGLE) then
                        NF90(nf90_put_att(ncid, meanvarids(i,j), '_FillValue', sFill))
   ...
   subroutine ncoutput(s,sl,par, tpar)
      ...
      use spaceparams,     only: indextos
      use timestep_module, only: timepars
      use mnemmodule,      only: arraytype, mnem_Subg, mnem_Sutot, mnem_Svtot, maxnamelen, mnem_alfaz, mnem_Susg
      use mnemmodule,      only: mnem_Svbg, mnem_Svsg, chartoindex, mnem_wetz
      use means_module,    only: meansparsglobal, meansparslocal
      use postprocessmod,  only: get_sister_mnem, gridrotate, postprocessvar_r2
      use spaceparamsdef,  only: spacepars
      use xmpi_module
      ...
      use spaceparams,     only: space_collect_mnem, space_local_to_global, space_collect_index
      use spaceparams,     only: space_who_has, space_global_to_local
      use means_module,    only: means_collect
#endif
      ...
      logical :: dooutput_global, dooutput_mean, dooutput_point, dooutput_drifter, dooutput_hotstart
      ...
      ! time for hotstart output?
      dooutput_hotstart = tpar%outputh .and. par%writehotstart == 1
      ...
      if (dooutput_hotstart) then
#ifdef USEMPI
         call hotstartfiles_collect(s,sl,par)
#endif
      endif
             ...
             case('r')
               select case(t%rank)
                ...
                case(2)
                  ...
                  if(par%remdryoutput==1) then
                     call postprocessvar_r2(s%wetz,t%name,r2,dFill)
                  endif
                  r2conv = CONVREAL(r2)
#ifdef USENETCDF
                  if(donetcdf) then
      ...
      if (dooutput_hotstart) then
         call hotstartfiles_write(s,par, tpar)
      endif
   ...
   subroutine points_output_init(s,par)
      ...
      use spaceparams
      use params,         only: parameters
      use postprocessmod, only: snappointstogrid
      use xmpi_module
   ...
   subroutine hotstartfiles_collect(s,sl,par)
      use logging_module
#ifdef USEMPI
      use xmpi_module
#endif
      ...
      ! Water level, bed, velocities. Used in all models
#ifdef USEMPI
      call space_collect_mnem(s,sl,par,mnem_zs)
      call space_collect_mnem(s,sl,par,mnem_zb)
      call space_collect_mnem(s,sl,par,mnem_uu)
      call space_collect_mnem(s,sl,par,mnem_vv)


      ! groundwater parameters
      if(par%gwflow==1) then
         call space_collect_mnem(s,sl,par,mnem_gwlevel)
         call space_collect_mnem(s,sl,par,mnem_gwhead)
         call space_collect_mnem(s,sl,par,mnem_gwcurv)
      endif

      ! wave energy
      if (par%swave==1) then
         call space_collect_mnem(s,sl,par,mnem_ee)
         call space_collect_mnem(s,sl,par,mnem_rr)
      endif

      ! single-dir
      if (par%single_dir==1) then
         call space_collect_mnem(s,sl,par,mnem_ee_s)
      endif

      ! nonh parameters
      if (par%wavemodel == WAVEMODEL_NONH) then
         call space_collect_mnem(s,sl,par,mnem_breaking)
         call space_collect_mnem(s,sl,par,mnem_wb)
         call space_collect_mnem(s,sl,par,mnem_ws)
      endif
      ...
      ! turbulence
      if (par%turb==1) then
         call space_collect_mnem(s,sl,par,mnem_kturb)
      endif

      ! structures
      if (par%struct==1) then
         call space_collect_mnem(s,sl,par,mnem_structdepth)
      endif

      ! nhplus
      if (par%nonhq3d==1) then
         call space_collect_mnem(s,sl,par,mnem_dU)
         call space_collect_mnem(s,sl,par,mnem_dV)
      endif
   ...
   end subroutine hotstartfiles_collect
   ...
   subroutine hotstartfiles_write(s,par, tpar)
      ...
      if (xomaster) then
         ! Water level, bed, velocities. Used in all models
         if (xomaster) then
            call lowlevel_write_hotstart_2d(s%zs,s%nx,s%ny,'zs',tpar%ith)
            call lowlevel_write_hotstart_2d(s%zb,s%nx,s%ny,'zb',tpar%ith)
            call lowlevel_write_hotstart_2d(s%uu,s%nx,s%ny,'uu',tpar%ith)
            call lowlevel_write_hotstart_2d(s%vv,s%nx,s%ny,'vv',tpar%ith)
         endif

         ! groundwater parameters
         if(par%gwflow==1) then
            call lowlevel_write_hotstart_2d(s%gwlevel,s%nx,s%ny,'gwlevel',tpar%ith)
            call lowlevel_write_hotstart_2d(s%gwhead,s%nx,s%ny,'gwhead',tpar%ith)
            call lowlevel_write_hotstart_2d(s%gwcurv,s%nx,s%ny,'gwcurv',tpar%ith)
         endif

         ! wave energy
         if (par%swave==1) then
            call lowlevel_write_hotstart_3d(s%ee,s%nx,s%ny,s%ntheta,'ee',tpar%ith)
            call lowlevel_write_hotstart_3d(s%rr,s%nx,s%ny,s%ntheta,'rr',tpar%ith)
         endif

         ! single-dir
         if (par%single_dir==1) then
            call lowlevel_write_hotstart_3d(s%ee_s,s%nx,s%ny,s%ntheta_s,'ee_s',tpar%ith)
         endif

         ! nonh parameters
         if (par%wavemodel == WAVEMODEL_NONH) then
            call lowlevel_write_hotstart_2d_int(s%breaking,s%nx,s%ny,'breaking',tpar%ith)
            call lowlevel_write_hotstart_2d(s%wb,s%nx,s%ny,'wb',tpar%ith)
            call lowlevel_write_hotstart_2d(s%ws,s%nx,s%ny,'ws',tpar%ith)
         endif

         ! sediment transport
         if (par%sedtrans==1 .and. (par%sus==1 .or. par%bulk==1)) then
            call lowlevel_write_hotstart_3d(s%ccg,s%nx,s%ny,par%ngd,'ccg',tpar%ith)
         endif

         ! turbulence
         if (par%turb==1) then
            call lowlevel_write_hotstart_2d(s%kturb,s%nx,s%ny,'kturb',tpar%ith)
         endif

         ! structures
         if (par%struct==1) then
            call lowlevel_write_hotstart_2d(s%structdepth,s%nx,s%ny,'structdepth',tpar%ith)
         endif

         ! nhplus
         if (par%nonhq3d==1) then
            call lowlevel_write_hotstart_2d(s%dU,s%nx,s%ny,'dU',tpar%ith)
            call lowlevel_write_hotstart_2d(s%dV,s%nx,s%ny,'dV',tpar%ith)
         endif

         ! boundary flow
         if ((par%flow==1).or.(par%wavemodel==WAVEMODEL_NONH)) then
            call lowlevel_write_hotstart_2d(s%umean,s%nx,s%ny,'umean',tpar%ith)
            call lowlevel_write_hotstart_2d(s%vmean,s%nx,s%ny,'vmean',tpar%ith)
         endif
      endif   ! xomaster
   end subroutine hotstartfiles_write

   subroutine lowlevel_write_hotstart_2d(var,nx,ny,varname,ith)

      integer,intent(in)           :: nx,ny,ith
      real*8,dimension(nx+1,ny+1),intent(in) :: var
      character(*), intent(in)     :: varname

      integer                      :: unit,i,j,ierr
      character(128)               :: fname,rowfmt

      unit = outunit(1,'hotstart')
      ...
      do j=1,ny+1
         write(unit,FMT=rowfmt) (var(i,j), i=1,nx+1)
      enddo
      close(unit)

   end subroutine lowlevel_write_hotstart_2d

   subroutine lowlevel_write_hotstart_2d_int(var,nx,ny,varname,ith)

      integer,intent(in)           :: nx,ny,ith
      integer,dimension(nx+1,ny+1),intent(in) :: var
      character(*), intent(in)     :: varname

      integer                      :: unit,i,j,ierr
      character(128)               :: fname,rowfmt

      unit = outunit(2,'hotstart')
      write(fname,'(a,a,I6.6,a)') 'hotstart_',trim(varname),ith,'.dat'
   ...
   end subroutine lowlevel_write_hotstart_2d_int

   subroutine lowlevel_write_hotstart_3d(var,nx,ny,nd,varname,ith)

      integer,intent(in)           :: nx,ny,nd,ith
      real*8,dimension(nx+1,ny+1,nd),intent(in) :: var
      character(*), intent(in)     :: varname

      integer                      :: unit,i,j,k,ierr
      character(128)               :: fname,rowfmt

      unit = outunit(3,'hotstart')
      write(fname,'(a,a,I6.6,a)') 'hotstart_',trim(varname),ith,'.dat'
      write(rowfmt,'(a,I12,a)') '(',ny+1,'(1X,D))'
      open(unit,file=fname,form='FORMATTED', iostat=ierr)
      do k=1,nd
         do j=1,ny+1
            write(unit,FMT=rowfmt) (var(i,j,k), i=1,nx+1)
         enddo
      enddo
      close(unit)

   end subroutine lowlevel_write_hotstart_3d
