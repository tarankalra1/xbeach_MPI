...
contains
   ...
   subroutine spectral_wave_bc(s,par,curline)
      ...
      if (xmaster) then
         if (reuseall) then
            ...
            par%Hrms = combspec%hm0/sqrt(2.d0)
            ...
            call generate_wave_train_Fourier(wp,s)
   ...
   subroutine generate_combined_spectrum(specinterp,combspec,px,trepfac,Tm01switch)
      ...
      ! Compute wave height
      !
      m0 = sum(combspec%Sf)*combspec%df
      combspec%hm0 = 4*sqrt(m0)
   ...
   subroutine generate_wave_train_Fourier(wp,s)
   ...
   subroutine generate_ebcf(wp,s,par)
      ...
      integer                                      :: reclen,fid,nInBoundaryFile
      ...
      !
      !
      ! 1D alfa_E approach for long waves
      ...
      if(par%wbcScaleEnergy==1) then
         nInBoundaryFile = nint((wp%tslenbc-1)*wp%dtbc/wp%dtin) ! number of total time series in bcf (independent of dtin / dtbc differences)
         do iy=1,s%ny+1
            stdeta = sum(E_tdir(iy,1:nInBoundaryFile,:))*s%dtheta   ! sum energy
      ...
      if (wp%dtchanged) then
         ...
         enddo
         ! write to file
         do irec=1,wp%tslenbc+2 ! IMDB bugfix requires additional time step here
   ...
   subroutine generate_qbcf(wp,s,par)
      ...
      integer                                      :: K               ! copy of K
      integer                                      :: halflen,reclen,fid,status,nInBoundaryFile
   ...
   subroutine generate_nhtimeseries_file(wp,par)
      ...
      write(rowfmt,'(A,I5,A)') '(',1+5*(par%ny+1),'(1X,E18.9))'
   ...
   subroutine generate_admin_files(par,wp,rtbc_local,dtbc_local,maindir_local,spectrumendtimeold,spectrumendtime)
      ...
      if (par%nonhspectrum==0) then
         ...
         write(fidelist,'(f12.3,a,f12.3,a,f9.3,a,f9.5,a,f9.3,a,f11.5,a)') &
         & spectrumendtime,' ',rtbc_local,' ',dtbc_local,' ',par%Trep,' ',maindir_local,' ',par%Hrms,' '//trim(wp%Efilename)
         close(fidelist)
         ...
         write(fidqlist,'(f12.3,a,f12.3,a,f9.3,a,f9.5,a,f9.3,a,f11.5,a)') &
         & spectrumendtime,' ',rtbc_local,' ',dtbc_local,' ',par%Trep,' ',maindir_local,' ',par%Hrms,' '//trim(wp%qfilename)
         close(fidqlist)
         ...
         if(par%single_dir==1) then
            ...
            write(fideslist,'(f12.3,a,f12.3,a,f9.3,a,f9.5,a,f9.3,a,f11.5,a)') &
            & spectrumendtime,' ',rtbc_local,' ',dtbc_local,' ',par%Trep,' ',maindir_local,' ',par%Hrms,' '//trim(wp%Esfilename)
            close(fideslist)
         endif
      else
         if (bccount==1) then
         ...
         write(fidnhlist,'(f12.3,a,f12.3,a,f12.3,a,f12.3,a)',iostat=iostat) &
         & spectrumendtimeold,'   ',spectrumendtime,' ',par%Trep,' ',par%Hrms,' '//trim(wp%nhfilename)
         close(fidnhlist)
      end if
