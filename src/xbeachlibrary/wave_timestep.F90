...
contains
   subroutine wave(s,par)
      ...
      use wave_stationary_directions_module
      ...
      real*8,dimension(:,:),allocatable,save          :: gamma

      if (.not.allocated(gamma)) then
         allocate(gamma(s%nx+1,s%ny+1))
      endif
      ...
      if (par%oldhmin==1) then
         s%hstokes = max(s%hh,par%hmin)
      else
         gamma = s%H/s%hh
         where (gamma>1.d0)
            s%hstokes = par%deltahmin*(gamma-1.d0)*s%H+s%hh
         elsewhere
            s%hstokes = s%hh
         endwhere
      endif
      ...
      select case (par%wavemodel)
       case(WAVEMODEL_STATIONARY)
         ...
         if ((abs(mod(par%t,par%wavint))<0.001d0*par%dt) .or. s%newstatbc==1) then
            ...
            call wave_stationary_directions(s,par,0)
            s%newstatbc   = 0
         endif
       case(WAVEMODEL_SURFBEAT)
         if (par%single_dir==1) then
            ...
            if ((abs(mod(par%t,par%wavint))<0.001d0*par%dt) .or. s%newstatbc==1 .or. par%t==par%dt) then
               ...
               call wave_stationary_directions(s,par,1)
               s%newstatbc   = 0
            endif
            !
