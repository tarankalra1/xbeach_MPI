...
6              continue
         ...
         elseif (par%wbctype==WBCTYPE_TS_2 .and. xmpi_istop) then
            ...
            (par%wbctype==WBCTYPE_SWAN) .or. (par%wbctype==WBCTYPE_VARDENS) .or. (par%wbctype==WBCTYPE_REUSE)) then
               ...
               if (startbcf) then
                  ...
                  if (xmaster) then
                     do i=1,curline
                        read(53,*,iostat=ier)bcendtime,rt,dtbcfile,par%Trep,s%theta0,par%Hrms,ebcfname
                        if (ier .ne. 0) then
                           call report_file_read_error('ebcflist.bcf')
                        endif
                        read(54,*,iostat=ier)bcendtime,rt,dtbcfile,par%Trep,s%theta0,par%Hrms,qbcfname
                        if (ier .ne. 0) then
                           call report_file_read_error('qbcflist.bcf')
                        endif
                        if(par%single_dir==1 .and. par%wbctype==WBCTYPE_REUSE) then
                           read(55,*,iostat=ier)bcendtime,rt,dtbcfile,par%Trep,s%theta0,par%Hrms,esbcfname
                           if (ier .ne. 0) then
         ...
         (par%wbctype==WBCTYPE_SWAN) .or. (par%wbctype==WBCTYPE_VARDENS) .or. (par%wbctype==WBCTYPE_REUSE)) then
            if (startbcf) then
               if(xmaster) then
                  ...
                  do i=1,curline
                     read(53,*,iostat=ier)bcstarttime,bcendtime,par%Trep,par%Hrms,nhbcfname
                     if (ier .ne. 0) then
   ...
   subroutine flow_bc(s,par)
      ...
      integer                                     :: i,j,jj,j1,indt,jg
      real*8                                      :: alphanew,vert,windxnow,windynow,factime,hybrelerr
      real*8                                      :: udnusum,dnusum,vdnvsum,dnvsum,maxdzs0
      real*8                                      :: dzs0,zs01old,zs02old,zs03old,zs04old,dzs01,dzs02,dzs03,dzs04
      real*8,save                                 :: hybll,hybhl
      real*8,save                                 :: hybf1,hybf2,hybtavg,hybtavgtot
      real*8,dimension(2),save                    :: hybmeanzs,hybmaxzs,hybminzs,hybmeanzs0,hybmaxzs0,hybminzs0
      ...
      if (.not. allocated(ht)) then
         ...
         if (par%tidetype==TIDETYPE_INSTANT .or. par%tidetype==TIDETYPE_HYBRID) then
         ...
         hybminzs  = huge(0.d0)
         hybminzs0 = huge(0.d0)
         hybmeanzs = 0.d0 ! do not initialise to zs!
         hybmeanzs0 = 0.d0
      endif
      ...
      ! for hybrid tide type
      if (par%epsi<0.d0 .and. par%swave==1 .and. par%tidetype==TIDETYPE_HYBRID) then
         if(hybtavgtot<hybtavg) then
            ! keep track of averages, min and max and hybtavgtot
            if (xmpi_istop) then
               hybmaxzs(1) = max(hybmaxzs(1),maxval(s%zs(1,jmin_zs:jmax_zs)))
               hybminzs(1) = min(hybmaxzs(1),minval(s%zs(1,jmin_zs:jmax_zs)))
               hybmaxzs0(1) = max(hybmaxzs0(1),maxval(s%zs0(1,jmin_zs:jmax_zs)))
               hybminzs0(1) = min(hybmaxzs0(1),minval(s%zs0(1,jmin_zs:jmax_zs)))
               ! Robert: okay, this is not a perfect average, because time will not perfectly match hybtavg.
               ! But! Small error (in both zs and zs0) and much easier than controling time step over simulation
               ! with time-varying Trep.
               hybmeanzs(1) = hybmeanzs(1)+par%dt/hybtavg* &
               sum(s%zs(1,jmin_zs:jmax_zs)*s%dnz(1,jmin_zs:jmax_zs))/sum(s%dnz(1,jmin_zs:jmax_zs))
               hybmeanzs0(1)= hybmeanzs0(1)+par%dt/hybtavg* &
               sum(s%zs0(1,jmin_zs:jmax_zs)*s%dnz(1,jmin_zs:jmax_zs))/sum(s%dnz(1,jmin_zs:jmax_zs))
            endif
            if (xmpi_isbot) then
               hybmaxzs(2) = max(hybmaxzs(2),maxval(s%zs(s%nx+1,jmin_zs:jmax_zs))) !ML was (s%ny+1,jmin_zs:jmax_zs) but that seems to have been a bug
               hybminzs(2) = min(hybmaxzs(2),minval(s%zs(s%nx+1,jmin_zs:jmax_zs)))
               hybmaxzs0(2) = max(hybmaxzs0(2),maxval(s%zs0(s%nx+1,jmin_zs:jmax_zs)))
               hybminzs0(2) = min(hybmaxzs0(2),minval(s%zs0(s%nx+1,jmin_zs:jmax_zs)))
               hybmeanzs(2) = hybmeanzs(2)+par%dt/hybtavg* &
               sum(s%zs(s%nx+1,jmin_zs:jmax_zs)*s%dnz(s%nx+1,jmin_zs:jmax_zs))/sum(s%dnz(s%nx+1,jmin_zs:jmax_zs))
               hybmeanzs0(2)= hybmeanzs0(2)+par%dt/hybtavg* &
               sum(s%zs0(s%nx+1,jmin_zs:jmax_zs)*s%dnz(s%nx+1,jmin_zs:jmax_zs))/sum(s%dnz(s%nx+1,jmin_zs:jmax_zs))
            endif
            hybtavgtot = hybtavgtot+par%dt
         else
            ! recompute hybf1 and hybf2
            if (xmpi_istop) then
               ! relative difference in zs and zs0 compared to "variance" of zs and zs0
               hybrelerr = abs(hybmeanzs(1)-hybmeanzs0(1))/min(huge(0.d0),max(hybmaxzs(1)-hybminzs(1),hybmaxzs0(1)-hybminzs0(1)))
               if (hybrelerr<hybll) then
                  hybf1 = 0.d0
               elseif (hybrelerr>hybhl) then
                  hybf1 = 1.d0
               else
                  ! Set to a cosine value between 0 and 1
                  hybf1 = 0.5d0-0.5d0*cos((hybrelerr-hybll)*par%px/(hybhl-hybll))
               endif
            else
               hybf1 = 0.d0 ! needed for MPI collection later
            endif
            if (xmpi_isbot) then
               ! relative difference in zs and zs0 compared to "variance" of zs and zs0
               hybrelerr = abs(hybmeanzs(2)-hybmeanzs0(2))/min(huge(0.d0),max(hybmaxzs(2)-hybminzs(2),hybmaxzs0(2)-hybminzs0(2)))
               if (hybrelerr<hybll) then
                  hybf2 = 0.d0
               elseif (hybrelerr>hybhl) then
                  hybf2 = 1.d0
                  ...
                  ! Set to a cosine value between 0 and 1
                  hybf2 = 0.5d0-0.5d0*cos((hybrelerr-hybll)*par%px/(hybhl-hybll))
               endif
            else
               hybf2 = 0.d0
            endif
            ! Reset means, min and max
            if (xmpi_istop .or. xmpi_isbot) then
               hybmaxzs  = -huge(0.d0)
               hybmaxzs0 = -huge(0.d0)
               hybminzs  = huge(0.d0)
               hybminzs0 = huge(0.d0)
            ...
            hybtavgtot = 0.d0
            !
            ! distribute coefficients if in MPI mode to maximum of each subdomain.
            ! note, all non-xmpitop/bot are deliberately set at 0 above to stop them interfering in further updates of MPI_MAX
#ifdef USEMPI
            call xmpi_allreduce(hybf1,MPI_MAX)
            call xmpi_allreduce(hybf2,MPI_MAX)
#endif
         endif
      endif
      !
      ...
      if (par%tidetype==TIDETYPE_INSTANT .or. par%tidetype==TIDETYPE_HYBRID) then
         ...
         ! these are the tides at the four model corner points
         zs01old = s%zs01
         zs02old = s%zs02
         zs03old = s%zs03
         zs04old = s%zs04
         call tide_boundary_timestep(s,par) ! also updates zs0 across domain
         ! tide change at the four model corner points (used to maximise change in the domain)
         dzs01 = s%zs01-zs01old
         dzs02 = s%zs02-zs02old
         dzs03 = s%zs03-zs03old
         dzs04 = s%zs04-zs04old
         do j=jmin_zs,jmax_zs
            do i=imin_zs,imax_zs
               if (s%wetz(i,j)==1 .and. s%zb(i,j)<s%zs0(i,j)) then  ! point is wet AND bed lower than zs0
                  if (minval(s%wetz(1:i,j))+minval(s%wetz(i:s%nx+1,j))>0) then !point connected front and/or back boundary by water
                     dzs0 = s%zs0(i,j)-zs0old(i,j)
                     ! in overwash situations dzs0 can shoot up because zs0 becomes connected to offshore
                     ! limit change to largest tide change at the boundary
                     if (dzs0>0.d0) then
                        dzs0 = min(dzs0,maxval( (/dzs01,dzs02,dzs03,dzs04/) ) )
                     elseif (dzs0<0.d0) then
                        dzs0 = max(dzs0,minval( (/dzs01,dzs02,dzs03,dzs04/) ) )
                     endif
                     do jg = 1,par%ngd
                        s%ccg(i,j,jg) = s%ccg(i,j,jg)*max(s%zs(i,j)-s%zb(i,j),par%eps)/max(s%zs(i,j)+dzs0-s%zb(i,j),par%eps)
                        s%ccg(i,j,jg) = min(s%ccg(i,j,jg),par%cmax)
                     enddo
                     s%zs(i,j) = s%zs(i,j)+dzs0
                  endif
               endif
            enddo
         enddo
      ...
      if (par%epsi<0.d0) then
         if (par%swave==1) then
            if(par%tidetype==TIDETYPE_VELOCITY) then
               factime = 1.d0/par%cats/par%Trep*par%dt
            elseif (par%tidetype==TIDETYPE_HYBRID) then
               factime = hybf1*1.d0/par%cats/par%Trep*par%dt
            else
               ! do we need this?
               factime = 0.1d0/par%cats/par%Trep*par%dt
            endif
         else
      ...
      if (factime>0.d0 .and. (par%tidetype==TIDETYPE_VELOCITY .or. par%tidetype==TIDETYPE_HYBRID)) then
         if (xmpi_istop) then
...
1000              continue
      ...
      !Dano: compute umean, vmean only at this location, same for all options; MPI compliant
      !
      ! modify factime for hybrid tide approach
      if(par%epsi<0.d0 .and. par%swave ==1 .and. par%tidetype==TIDETYPE_HYBRID) then
         factime = hybf2*1.d0/par%cats/par%Trep*par%dt
      endif
