...
contains
   ...
   subroutine roelvink_1D(par,s,i)
      ...
      integer                                         :: j
      real*8, dimension(:),allocatable,save           ::kmr,arg,H,hr ! change to allocatable save to reduce overhead time
      
      if (.not. (allocated(kmr))) then
         allocate(kmr(s%ny+1))
         allocate(arg(s%ny+1))
         allocate(H(s%ny+1))
         allocate(hr(s%ny+1))
      ...
      if (par%break /= BREAK_ROELVINK_DALY) then
         ...
         do j=1,s%ny+1
            if (s%wete(i,j)==1) then
               if (H(j) > par%gamma *hr(j)) then
                  s%Qb(i,j) = 1.d0
               elseif (H(j) < par%gamma2*hr(j)) then
                  s%Qb(i,j) = 0.d0
               endif ! else keep value of Qb (advected)
            endif
         enddo
      endif
   ...
   subroutine roelvink_2D(par,s)
      ! moved back to 2D-specific subroutine to remove large outer-loop memory overhead problem of calling
      ! do i=1,s%nx+1
      ...
      integer                                         :: i,j
      real*8, dimension(:,:),allocatable,save         :: kmr,arg ! change to allocatable save to reduce overhead time
      ...
      if (par%break /= BREAK_ROELVINK_DALY) then
         if (par%wci==1) then
            arg = -( s%H / (par%gamma*tanh(kmr*s%hhw)/kmr))**par%n
         else
            arg = -( s%H / (par%gamma*s%hhw              ))**par%n
         endif
         ...
         do j=1,s%ny+1
            do i=1,s%nx+1
               if (s%wete(i,j)==1) then
                  if (s%H(i,j) > par%gamma *s%hhw(i,j)) then
                     s%Qb(i,j) = 1.d0
                  elseif (s%H(i,j) < par%gamma2*s%hhw(i,j)) then
                     s%Qb(i,j) = 0.d0
                  endif ! else keep value of Qb (advected)
               endif
            enddo
      ...
      s%D = s%Qb * 2.d0 * par%alpha * s%E

      if (par%break == BREAK_ROELVINK1) then
         if (par%wci==1) then
            s%D = s%D * s%sigm/2.d0/par%px;
         else
            s%D = s%D / par%Trep
         endif
      elseif (par%break == BREAK_ROELVINK2 .or. par%break == BREAK_ROELVINK_DALY) then
         ! Jaap: also wci switch for roelvink2
         if (par%wci==1) then
            s%D = s%D * s%sigm/2.d0/par%px * s%H/s%hh
         else
            s%D = s%D / par%Trep * s%H/s%hh
         endif
      end if
   ...
   subroutine baldock_1D(par,s,i)
      ...
      real*8,dimension(:),allocatable,save   :: kh,f,k,H,Hb,R,gamma
      
      if (.not. (allocated(kh))) then
         allocate(kh(s%ny+1))
         allocate(f(s%ny+1))
         allocate(k(s%ny+1))
         allocate(H(s%ny+1))
   ...
   subroutine janssen_battjes_1D(par,s,i)
      ...
      real*8, dimension(:),allocatable,save           :: kh,f,k,H,Hb,R
