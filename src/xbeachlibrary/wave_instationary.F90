   ...
   subroutine wave_instationary(s,par)
      ...
      select case(par%break)
       ...
       case(BREAK_ROELVINK_DALY)  ! Dano: probably not MPI ok
         cgxm = s%c*dcos(s%thetamean-s%alfaz)
         call advecqx(cgxm,s%Qb,xwadvec,s%nx,s%ny,s%dsu,s%wete,par%oldRD1D)
         if (s%ny>0) then
            cgym = s%c*dsin(s%thetamean-s%alfaz)
            call advecqy(cgym,s%Qb,ywadvec,s%nx,s%ny,s%dnv,s%wete,par%oldRD1D)
         ...
         where(s%wete==0)
            s%Qb=0.d0
         elsewhere
            s%Qb = min(max(s%Qb, 0.d0),1.d0) ! ensure between 0 and 1
         endwhere
#ifdef USEMPI
         call xmpi_shift_ee(s%Qb)
#endif
         call roelvink(par,s)
      end select
