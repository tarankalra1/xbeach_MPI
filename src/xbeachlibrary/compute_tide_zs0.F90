...
contains
         ...
         elseif (tide1>maxzbg .and. tide2<=maxzbg) then
            select case (boundaryID)
             ...
             case (3,4) ! left,right
               ddist = 1.d0/(disttide2-distmaxzbg)
               do i = imin,imax
                  if (s%sdist(i,jmin)<=distmaxzbg) then
                     s%zs0(i,jmin) = tide1
   ...
   subroutine fill_tide_grid(s)
      ! This subroutine fills the zs0 field. Used by tide=instant and hybrid, and by tide=velocity at initialization
      ! subroutine is not yet fully MPI-compliant for tide=instant/hybrid (only works if mpi-domains are cross-shore strips)
      use spaceparamsdef,   only: spacepars
