...
contains
   subroutine compute_wetcells(s,par)
      ...
      do j=1,s%ny+1
         do i=1,s%nx+1
            if(s%hu(i,j)>par%eps+numeps .and. s%hum(i,j)>par%eps+numeps) then  ! Jaap and Pieter: If you want to compute correct advection term
      ...
      do j=1,s%ny+1
         do i=1,s%nx+1
            if(s%hv(i,j)>par%eps+numeps .and. s%hvm(i,j)>par%eps+numeps) then
               s%wetv(i,j)=1
            else
               s%wetv(i,j)=0
            end if
      ...
      do j=1,s%ny+1
         do i=1,s%nx+1
            ...
            if(s%hh(i,j)>par%eps+numeps) then
               s%wetz(i,j)=1
            else
               s%wetz(i,j)=0
            end if
