module filefunctions
   ...
   implicit none
   private
   public create_new_fid, checkbcfilelength, get_file_length, check_file_exist, check_file_length
   save
...
contains
   ...
   subroutine checkbcfilelength(tstop,wbctype,filename,filetype,nonh)
      ...
      real*8            :: t,dt,total,d1,d2,d3,d4,d5,d6
         ...
         do ifid=1,nlocs
            ...
            select case (filetype)
             ...
             case (3)
               do while (total<tstop .and. i<bcfiles(ifid)%nlines)
                  if (lnonh) then
                     read(fid,*,iostat=ier)d1,total,d2,d3,dummy
                  else
                     read(fid,*,iostat=ier)total,d2,d3,d4,d5,d6,dummy
                  endif
                  if (ier .ne. 0) then
                     call report_file_read_error(bcfiles(ifid)%fname)
                  endif
