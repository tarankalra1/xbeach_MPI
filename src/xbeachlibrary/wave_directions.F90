...
contains
   ...
   subroutine wave_directions(s,par)
      ...
      use wave_functions_module, only: slope2d, advecthetaho, advecxho, advecyho, compute_wave_direction_velocities
      use xmpi_module
      ...
      real*8 , dimension(:),allocatable,save      :: Hprev,thetaprev
      ...
      if (.not. allocated(e01)) then
         ...
         if(par%cyclic==1 .and. scheme_local .ne. SCHEME_UPWIND_1) then
      ...
      do i=2,imax
         ...
         if(par%wci==0) then
            ...
            dtw=min(dtw,.5d0*s%dtheta_s                            /max(1.0d-6,maxval(abs(s%ctheta_s(i,jmin_ee:jmax_ee,:)   ))) )
         ...
         do while (stopiterate .eqv. .false.)
            ...
            if (s%ny>0) then
               ...
               0,s%ny,s%ntheta_s,s%dsv(i,:),s%dnv(i,:),s%dsdnzi(i,:),scheme_local_yadvec,s%wete(i,:),dtw,s%dnz)
            endif
            ! Robert: could this also be warm-beam?
            call advecthetaho(s%ee_s(i,:,:),s%ctheta_s(i,:,:),thetaadvec(i,:,:),0,s%ny,s%ntheta_s, &
            s%dtheta_s,scheme_local,s%wete(i,:))
            ...
            enddo
#ifdef USEMPI
            call xmpi_shift(s%ee_s(i-1:i+1,:,:),SHIFT_Y_R,1,2)
            call xmpi_shift(s%ee_s(i-1:i+1,:,:),SHIFT_Y_L,3,4)
#endif
            !
            ! Energy integrated over wave directions,Hrms
            !
            forall(j=1:s%ny+1,s%wete(i,j)==1)
               s%E(i,j)=sum(s%ee_s(i,j,:))*s%dtheta_s
            endforall
            where(s%wete(i,:)==1)
            ...
            ! check with gammax
            do j=1,s%ny+1
               if (s%wete(i,j)==1 .and. s%H(i,j)>par%gammax*s%hhws(i,j)) then
                  do itheta=1,s%ntheta_s
                     s%ee_s(i,j,itheta)=s%ee_s(i,j,itheta)/(s%H(i,j)/(par%gammax*s%hhws(i,j)))**2
                  enddo
                  s%H(i,j)=par%gammax*s%hhws(i,j)
                  s%E(i,j)=par%rhog8*s%H(i,j)**2
               endif
            enddo
            !
            ...
            if (xmpi_isleft .and. s%ny>0) then
               do itheta=1,s%ntheta_s
                  !if (s%sinth_s(i,1,itheta)>=0.) then  ! Robert: this is different to wave_stationary
            ...
            !
            where(s%wete(i,:)==1)
               s%E(i,:)=sum(s%ee_s(i,:,:),2)*s%dtheta_s
               s%H(i,:)=sqrt(s%E(i,:)/par%rhog8)
            elsewhere
               s%E(i,:)=0.d0
               s%H(i,:)=0.d0
