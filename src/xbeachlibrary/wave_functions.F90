module wave_functions_module
   ...
   save
   private
   public:: dispersion, slope2d, breakerdelay, &
   &        advecthetaho, advecxho, advecyho, iteratedispersion, advecqy, advecwy, &
   &        advecwx, advecqx, wave_dispersion, update_means_wave_flow, compute_wave_direction_velocities, compute_stokes_drift, compute_wave_forces
...
contains
   ...
   subroutine update_means_wave_flow(s,par)
      ...
      if (par%wavemodel == WAVEMODEL_SURFBEAT .and. par%single_dir == 1) then
         ...
         s%hhws = max(factime*s%hhw + (1-factime)*s%hhws,par%eps)
         if (par%wci==1) then
      ...
      if (par%wavemodel == WAVEMODEL_SURFBEAT .and. par%wci==1) then
         ...
         s%hhwcins = max(factime*s%hhw + (1-factime)*s%hhwcins,par%eps)
   ...
   subroutine advecxho(ee,cgx,xadvec,nx,ny,ntheta,dnu,dsu,dsdnzi,scheme,wete,dt,dsz)
      ...
      integer                                         :: scheme_now
      integer                                         :: imin_local,imax_local

      if(nx>3) then
         imin_local = imin_ee
         imax_local = imax_ee
      else
         ! Need nx here in case of stationary wave solver (nx=3 given in subroutine call; arrays from (i-2:i+1), need to fill
         ! 3rd place in the arrays (i) )
         imin_local = nx
         imax_local = nx
      endif
   ...
   subroutine advecthetaho(ee,ctheta,thetaadvec,nx,ny,ntheta,dtheta,scheme,wete)
      ...
      if(nx>0) then
         ...
         ! Need from 1 here in case of stationary wave solver (nx=0 given in subroutine call)
         imin_local = 1
         imax_local = 1
      endif
      ...
      if (ntheta>1) then
         ...
         !        do i=imin_local,imax_local
         !           if(wete(i,j)==1) then
         !              thetaadvec(i,j,itheta)=   thetaadvec(i,j,itheta)             &
         !                                        - (ee(i,j,itheta+1)-2*ee(i,j,itheta)+ee(i,j,itheta-1))/ &
         !                                         dtheta**2*dt/2*ctheta(i,j,itheta)**2
         !           endif
         !        enddo
         !     enddo
         !  enddo
   ...
   subroutine advecqx(c,arrin2d,xwadvec,nx,ny,dsu,wete,old1Dadvection) !,cyclic)
      use spaceparams

      implicit none

      integer,intent(in)                              :: nx,ny,old1Dadvection !,cyclic
      ...
      if(old1Dadvection==1) then ! this is old, and incorrect implementation. But keep backward-compatibility
         j1=2
         j2=ny
         i1=2
         i2=nx
   ...
   subroutine advecqy(c,arrin2d,ywadvec,nx,ny,dnv,wete,old1Dadvection)
      use spaceparams
      implicit none

      integer                                         :: nx,ny,old1Dadvection
      integer                                         :: i,j,j1,j2,i1,i2
      real*8 , dimension(nx+1,ny+1)                   :: dnv
   ...
   subroutine breakerdelay(par,s)
      !!! Dano: this routine is not MPI compliant and assumes xz to be in cross-shore; TBD
      !!! --------------------------------------------------------------------------------
      use params,         only: parameters
      use spaceparams
      use xmpi_module
      ...
      real*8                                          :: Lbr
      real*8, dimension(:),allocatable,save           :: utemp
      integer                                         :: jx,jy,i,j1,nbr,tempxid
      integer, dimension(:),allocatable,save          :: ibr
      
      if (.not.allocated(ibr)) then
         allocate(ibr(s%nx+1))
         allocate(utemp(s%nx+1))
      endif
   ...
   subroutine compute_wave_forces(s)
      use spaceparams
   ...
   end subroutine compute_wave_forces

   subroutine compute_stokes_drift(s,par)
      use params
      use spaceparams
      ...
      real*8 , dimension(:,:)  ,allocatable,save  :: ustw

      if (.not. allocated(ustw)) then
         allocate(ustw        (s%nx+1,s%ny+1))
      endif

      where(s%wete==1)
         ! wave induced mass flux
         ustw=s%E/s%c/par%rho/s%hstokes   ! Jaap (Robert: used in wave instationary)
         ! ustw=s%E*s%k/s%sigm/par%rho/max(s%hh,.001d0)   ! Robert: old version used in wave stationary. Superceded?
         s%uwf = ustw*cos(s%thetamean-s%alfaz)
         s%vwf = ustw*sin(s%thetamean-s%alfaz)
         !
         ! roller contribution
         s%ustr=2*s%R/s%c/par%rho/s%hstokes ! Jaap (Robert: used in wave instationary)
         !s%ustr = 0.d0
         !s%ustr=2*s%R*s%k/s%sigm/par%rho/max(s%hh,.001d0) ! Robert: old version used in wave stationary. Superceded?
      elsewhere
         ustw = 0.d0
         s%uwf = 0.d0
         s%vwf = 0.d0
   ...
   end subroutine compute_stokes_drift
