...
contains
   subroutine transus(s,par)
      ...
      s%Dc = par%facDc*s%nuh
      !Robert: removed this which is not grid-independent s%Dc = (par%nuh+par%nuhfac*s%hh*(s%DR/par%rho)**(1.d0/3.d0))
      ...
      do jg = 1,par%ngd
         ...
         cc(imin_zs:imax_zs,jmin_zs:jmax_zs) = cc(imin_zs:imax_zs,jmin_zs:jmax_zs)/s%hh(imin_zs:imax_zs,jmin_zs:jmax_zs)
   ...
   subroutine bed_update(s,par)
      ...
      integer                                     :: n_aval,im1,ip1,jm1,jp1
      real*8,save                                 :: delta
      real*8                                      :: totslp,Sig
      ...
      if (par%t>=par%morstart .and. par%t < par%morstop .and. par%morfac > .999d0) then
         ...
         call xmpi_shift_ee(s%structdepth)
#endif
         ...
         if(par%gwflow==1) then
            ...
            call xmpi_shift_ee(s%infil)
            call xmpi_shift_ee(s%zs)
#endif
         else ! only need to update water levels
            where (s%wetz(imin_zs:imax_zs,jmin_zs:jmax_zs)==1)
               s%zs(imin_zs:imax_zs,jmin_zs:jmax_zs) = s%zs(imin_zs:imax_zs,jmin_zs:jmax_zs)+s%dzbnow(imin_zs:imax_zs,jmin_zs:jmax_zs)
            endwhere
#ifdef USEMPI
            call xmpi_shift_ee(s%zs)
#endif
         ...
         if (par%avalanching==1) then
            ...
            if(par%fixedavaltime==0) then
               par%avaltime = par%nTrepavaltime*par%Trep
            endif
            !
            ! Estimate IG swash excursion based on Stockdon 2006
            ! Sig = 0.06 * sqrt(H0L0) -> H0 ~ Hs =sqrt(2) x Hrms -> Sig = 0.06 *(2)^(1/4) * sqrt(HrmsL0) ~ 0.07 * sqrt(HrmsL0)
            !                         -> L0 ~ g*(1.1*Trep)^2/2/pi -> Sig = 0.06*(2)^(1/4)*1.1*sqrt(Hrms*g*Trep^2/2/pi)
            Sig = 0.078*sqrt(par%Hrms*par%g*par%Trep**2/2/par%px)
            ...
            do ii=1,nint(par%morfac)
               ...
               do j=1,s%ny+1
                  ...
                  enddo
                  s%dzbdx(s%nx+1,j)=s%dzbdx(s%nx,j)
               enddo
               do j=1,s%ny
                  do i=1,s%nx+1
               ...
               if (s%ny>0) then
                  do i=1,s%nx+1
                     s%dzbdy(i,s%ny+1)=s%dzbdy(i,s%ny)
                  enddo
               endif
               !
               hav = s%hh
               ...
               do i=2,s%nx !-1 Jaap -1 gives issues for bed updating at mpi boundaries
                  do j=1,s%ny+1
                     ...
                     ip1=min(i+1,s%nx+1)
                     jm1=max(j-1,1)
                     totslp=sqrt(s%dzbdx(i,j)**2+(.25*(s%dzbdy(i,j)+s%dzbdy(ip1,j)+s%dzbdy(i,jm1)+s%dzbdy(ip1,jm1)))**2)
                     if(max(hav(i,j),hav(i+1,j))>par%hswitch+par%eps) then ! Jaap instead of s%hh
                        dzmax=par%wetslp*abs(s%dzbdx(i,j))/max(totslp,tiny(0.d0))
                        ...
                        dzmax=par%dryslp*abs(s%dzbdx(i,j))/max(totslp,tiny(0.d0))
                     end if
                     ...
                     if(abs(s%dzbdx(i,j))>dzmax+1.d-10 .and. s%structdepth(i+nint(max(0.d0,sign(1.d0,s%dzbdx(i,j)))),j)>par%eps) then
                        ...
                        ! Robert: something like this needed to remove grid-dependency
                        dzb = dzb*par%dt/par%avaltime*max((Sig/s%dsu(i,j))**2,1.d0)
                        ...
                        if (dzb >= 0.d0) then
                           ...
                           !dzb=dzb/par%avaltime*par%dt                          ! Dano more transparent formulation, removes direction dependence
               ...
               do j=2,s%ny !-1 Jaap -1 gives issues for bed updating at mpi boundaries
                  do i=1,s%nx+1
                     jp1=min(j+1,s%ny+1)
                     im1=max(i-1,1)
                     totslp=sqrt(s%dzbdy(i,j)**2+(.25*(s%dzbdx(i,j)+s%dzbdx(i,jp1)+s%dzbdx(im1,j)+s%dzbdx(im1,jp1)))**2)

                     if(max(s%hh(i,j),s%hh(i,j+1))>par%hswitch+par%eps) then
                        dzmax=par%wetslp*abs(s%dzbdy(i,j))/max(totslp,tiny(0.d0))
                     else
                        dzmax=par%dryslp*abs(s%dzbdy(i,j))/max(totslp,tiny(0.d0))
                     end if
                     if(abs(s%dzbdy(i,j))>dzmax .and. s%structdepth(i,j+nint(max(0.d0,sign(1.d0,s%dzbdy(i,j)))))>par%eps) then ! Jaap
                        ...
                        ! Robert: something like this needed to remove grid-dependency
                        dzb = dzb*par%dt/par%avaltime*(Sig/s%dnv(i,j))**2
                        !
                        if (dzb >= 0.d0) then
                           ...
                           !dzb=dzb/par%avaltime*par%dt                         ! Dano
   ...
   subroutine sedtransform(s,par)
      ...
      if (par%swave==1) then
         ...
         do j=1,s%ny+1
            do i=1,s%nx+1
               ...
               dcfin = exp(min(100.d0,hloc(i,j)/max(ML,0.00000000000000001d0)))
      ...
      do jg = 1,par%ngd
         ...
         if (par%oldTsmin==1) then
            s%Tsg(:,:,jg) = max(Ts,par%Tsmin)
         else
            s%Tsg(:,:,jg) = max(Ts,par%dtlimTs*par%dt)
         endif
         !
         ...
         if (par%form==FORM_SOULSBY_VANRIJN) then       ! Soulsby van Rijn
            ...
            term1=sqrt(term1)
            !
            ceqb = 0.d0                                                                    !initialize ceqb
            ceqs = 0.d0                                                                     !initialize ceqs
            do j=1,s%ny+1
               do i=1,s%nx
                  ! Lodewijk: separate bed load from suspended load since Ucr not anymore the same
         ...
         !
         ! DurosTA-type alfaD50 hack
         if (par%alfaD50 > 0) then
            ceqb =  ceqb * (0.000225/par%D50(jg))**par%alfaD50
            ceqs =  ceqs * (0.000225/par%D50(jg))**par%alfaD50
         endif
         !
      ...
      do jg = 1,par%ngd
         ...
         if (par%bulk==0) then
            ...
            if (par%oldTsmin==1) then
               ...
               s%Tsg(:,:,jg) = max(par%tsfac*s%hu/w(jg),par%dtlimTs*par%dt)
            endif
            !s%Tsg(:,:,jg) = max(par%tsfac*s%hu/w(jg),par%Tsmin)
   ...
   subroutine waveturb(s,par)
      ...
      use spaceparams
      use xmpi_module
      use paramsconst, only: TURBADV_EULERIAN, TURBADV_LAGRANGIAN, TURBADV_NONE
      ...
      if (par%turbadv == TURBADV_NONE) then
         ...
         if (s%ny>0) then
            do j=2,s%ny+1
               do i=2,s%nx+1
                  if (par%betad>0) then
                     ...
                     !ML=max(s%rolthick(i,j),0.07d0*s%hstokes(i,j))
                     !disturb=0.08d0*s%hstokes(i,j)/ML*s%kturb(i,j)**1.5d0
                  endif
            ...
            j=1
            do i=2,s%nx+1
               ! Robert: perhaps better to re-write to implicit sink term
               s%kturb(i,j) = (s%hold(i,j)*s%kturb(i,j)-par%dt*(       &
               (Sturbu(i,j)*s%dnu(i,j)-Sturbu(i-1,j)*s%dnu(i-1,j))*s%dsdnzi(i,j)-&
               (ksource(i,j)-par%betad*s%kturb(i,j)**1.5d0)))/s%hh(i,j)
               s%kturb(i,j)=max(s%kturb(i,j),0.0d0)

            enddo
         endif

         !Robert: cannot divide by hh here again
         !s%kturb = s%kturb/s%hh !max(s%hh,0.01d0) :
   ...
   subroutine RvR(s,par)
      ...
      if (.not. allocated(Urs)) then
         ...
         ! MvdL: updated values to Ruessink et al. (2012) coastal engineering
         m1 = 0;       ! a = 0
         m2 = 0.857 !0.7939;  ! b = 0.79 +/- 0.023
         m3 = -0.471 !-0.6065; ! c = -0.61 +/- 0.041
         m4 = 0.297 !0.3539;  ! d = -0.35 +/- 0.032
         m5 = 0.815 !0.6373;  ! e = 0.64 +/- 0.025
         m6 = 0.672 !0.5995;  ! f = 0.60 +/- 0.043
      ...
      !Bm = m1 + (m2-m1)/(1.d0+beta*Urs**alpha)
      ...
      if (par%alfaD50 > 0) then
         s%ua =  s%ua * (par%D50(1)/0.000225)**par%alfaD50
      endif
   ...
   subroutine setbathy_update(s, par)
      ...
      integer                             :: i,j,dummy
      real*8,dimension(:,:),allocatable,save  :: zbnew
      
      if(.not.allocated(zbnew)) then
         allocate(zbnew(s%nx+1,s%ny+1))
      endif
