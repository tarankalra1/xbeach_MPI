...
#include "config.h"
#endif
   use ncoutput_module,  only: ncoutput, points_output_init, fortoutput_init
#ifdef USENETCDF
   use ncoutput_module,  only: ncoutput_init
#endif
   use params,           only: parameters
   use spaceparams
   use timestep_module,  only: timepars, outputtimes_update
   ...
   save
   private
   public output, output_error, output_init, log_progress
...
contains
   ...
   subroutine output(sglobal,s,par,tpar,update)
      ...
      ! write to log file is hotfile is generated
      if (tpar%outputh .and. par%writehotstart == 1) then
         call writelog('ls','','Writing hotstart file no. ',tpar%ith,' at t = ',par%t,' s')
      endif
   ...
   contains
      subroutine tell_xomaster_what_time_it_is
         ...
         call xmpi_send(xmpi_imaster,xmpi_omaster,tpar%ith)
         ...
         call xmpi_send(xmpi_imaster,xmpi_omaster,tpar%outputh)
