   ...
   subroutine bedroughness_update(s,par)
      ...
      select case (par%bedfriction)
       ...
       case (BEDFRICTION_MANNING)
         ...
         if (par%dynamrough .eq. 1)  then
            ! ML 2 KN changed back to one where loop instead of 3 separate to ensure just 1 of the 3 statements is executed per cell.
            where ((isveggie) .and. (s%sedero .gt. 0d0))                      ! linear function do to deposition ( sedero > 0 )
               s%bedfriccoef    = nmanmin + min( max( (par%dstem-s%sedero)/par%dstem , 0.d0), 1.0d0) * (nman0 - nmanmin)
            elsewhere   ((isveggie) .and. (s%sedero .lt. (par%droot*-1) ) )   ! step function do to erosion larger than root than always minimum ( sedero < -droot )
               s%bedfriccoef    = nmanmin
               isveggie         = .false.
            elsewhere                                                         ! linear function do to deposition ( -droot < sedero < 0 )
               s%bedfriccoef    = nmanmin + min( max( (par%droot+s%sedero)/par%droot , 0.d0), 1.0d0) * (nman0 - nmanmin)
            endwhere
         endif
