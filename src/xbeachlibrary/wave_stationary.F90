module wave_stationary_module
   ...
   save
   private
   public:: wave_stationary
contains
   subroutine wave_stationary(s,par)
      use params,                only: parameters
      use spaceparams
      use roelvink_module,       only: roelvink, baldock, janssen_battjes
      use wave_functions_module, only: dispersion, slope2d, breakerdelay, &
      &                                advecthetaho, advecxho, advecyho, compute_wave_direction_velocities
      use xmpi_module
      ...
      integer,save                :: scheme_local_yadvec
      ...
      if (.not. allocated(xadvec)) then
         ...
         if(par%cyclic==1 .and. par%scheme .ne. SCHEME_UPWIND_1) then
            ...
            scheme_local_yadvec = par%scheme
         endif
      endif
      ...
      do i=2,imax
         ...
         do while (stopiterate .eqv. .false.)
            ...
            if (s%ny>0) then
               call advecyho(s%ee(i,:,:),s%cgy(i,:,:),yadvec(i,:,:),  & ! RMc: why was this always set to upwind_1?
               0,s%ny,s%ntheta,s%dsv(i,:),s%dnv(i,:),s%dsdnzi(i,:),scheme_local_yadvec,s%wete(i,:),dtw,s%dnz)
            endif
            ...
            !
            ! RMc: changed to match ee balance
            if  (i>2 .and. (par%scheme==SCHEME_UPWIND_2 .or. par%scheme==SCHEME_WARMBEAM)) then
               ! note: at the moment Warm-Beam is currently not a valid combination (automatic change to upwind_2 in params.F90)
