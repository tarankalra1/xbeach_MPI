...
contains
   subroutine flow(s,par)
      ...
      use spaceparamsdef
      use xmpi_module
      use boundaryconditions,      only: flow_lat_bc, discharge_boundary_h, discharge_boundary_v
      use flow_secondorder_module, only: flow_secondorder_huhv, flow_secondorder_advuv
      use nonh_module,             only: nonh_cor
      use bedroughness_module,     only: bedroughness_init, bedroughness_update
      use vsm_u_xb_module,         only: vsm_u_XB
      use paramsconst
      use rainfall_module
      use constants,               only: dFill
      ...
      !
      if (par%viscosity==0) then ! Dano: put it here so it actualy saves a lot of computation time
         s%nuh = 0.d0
         s%viscu = 0.d0
      else
         ...
         if (par%swave == 1) then
            if (par%smag==0) then ! approach of Reniers seems inconsistent with Smagorinski
               do j=j1,max(s%ny,1)
                  do i=2,s%nx
         ...
         !
         do j=jmin_uu,jmax_uu
            do i=imin_uu,imax_uu
         ...
         do j=jmin_uu,jmax_uu
            ...
            do i=imin_uu,imax_uu
               ...
               nuh2  = .25d0*(s%nuh(i,j)+s%nuh(i+1,j)+s%nuh(i+1,jm1)+s%nuh(i,jm1))

               dudy1 = nuh1 *.5d0*(s%hvm(i,j  )+s%hvm(i+1,j  ))*(s%uu(i,jp1)-s%uu(i,j))/s%dnc(i,j)
               dudy2 = nuh2 *.5d0*(s%hvm(i,jm1)+s%hvm(i+1,jm1))*(s%uu(i,j)-s%uu(i,jm1))/s%dnc(i,jm1)
               s%viscu(i,j) = s%viscu(i,j) + (1.0d0/s%hum(i,j))* &
               ( 2.0d0*(dudy1-dudy2)/(s%dnc(i,j)+s%dnc(i,jm1)) )*s%wetu(i,jp1)*s%wetu(i,jm1)
            end do
         end do
         ...
         if (par%smag == 1) then
            do j=jmin_uu,jmax_uu
               ...
               do i=imin_uu,imax_uu
                  ...
                  dvdx2 = nuh2*.5d0*(s%hvm(i,jm1)+s%hvm(i+1,jm1))*(s%vv(i+1,jm1)-s%vv(i,jm1))/s%dsc(i,jm1)
         ...
         !
         ! Viscosity y-direction
         !
         s%viscv =0.d0
         do j=jmin_vv,jmax_vv
            jp1=min(j+1,1)
            jm1=max(j-1,1)
            do i=imin_vv,imax_vv
               dvdy1 = s%nuh(i,jp1)*s%hh(i,jp1)*(s%vv(i,jp1)-s%vv(i,j))/s%dnz(i,jp1)
               dvdy2 = s%nuh(i,j)  *s%hh(i,j  )*(s%vv(i,j)-s%vv(i,jm1))/s%dnz(i,j)
               s%viscv(i,j) = (1.0d0/s%hvm(i,j))* 2*(dvdy1-dvdy2)/(s%dnz(i,j)+s%dnz(i,jp1))*s%wetv(i,jp1)*s%wetv(i,jm1)
            end do
         end do
      ...
      do j=jmin_vv,jmax_vv
         do i=imin_vv,imax_vv
            if(s%wetv(i,j)==1) then
               ...
               ! Dano: I don't agree
               dvdt = (s%udvdx(i,j)+s%vdvdy(i,j)-s%viscv(i,j)& !Ap,Robert,Jaap
                       + par%g*s%dzsdy(i,j)&
                       + s%tauby(i,j)/(par%rho*s%hv(i,j)) &  ! Dano: s%hv instead of s%hvm, NOT cf volume approach
                       + s%Fvegv(i,j)/(par%rho*s%hv(i,j)) &
                       - par%lwave*s%Fy(i,j)/(par%rho*s%hvm(i,j)) &
                       + fc*s%uv(i,j) &
                       - par%rhoa*par%Cd*s%windnv(i,j)*sqrt(s%windsu(i,j)**2+s%windnv(i,j)**2)/(par%rho*s%hvm(i,j)))    ! Kees: wind correction
               if (dvdt>par%maxfacg*par%g) then
                    dvdt=par%maxfacg*par%g
                elseif (dvdt<-par%maxfacg*par%g) then
                    dvdt=-par%maxfacg*par%g
                endif
               s%vv(i,j)=s%vv(i,j)-par%dt*dvdt
            else
               s%vv(i,j)=0.0d0
            end if
         end do
      ...
      ! update hu en hv for continuity
      do j=1,s%ny+1
         do i=1,s%nx+1
      ...
      s%qy=s%vv*s%hv
      !
      ! Compute depth-velocity product magnitude (purely for output)
      s%qmag=sqrt(s%qx**2+s%qy**2)
      !
      ...
      if (s%ny>0) then
         do j=jmin_zs,jmax_zs
            do i=imin_zs,imax_zs
               ...
               - s%infil(i,j) + s%rainfallrate(i,j)
            end do
         end do
         s%zs(imin_zs:imax_zs,jmin_zs:jmax_zs) = s%zs(imin_zs:imax_zs,jmin_zs:jmax_zs) &
         +s%dzsdt(imin_zs:imax_zs,jmin_zs:jmax_zs)*par%dt
      else
         j=1
         do i=imin_zs,imax_zs
         ...
         s%zs(imin_zs:imax_zs,1) = s%zs(imin_zs:imax_zs,1)+s%dzsdt(imin_zs:imax_zs,1)*par%dt
      endif !s%ny>0
      ...
      where(s%wetz==1)
         where(s%maxzs>dFill+1.d0) ! points that have been wet before
      ...
      if (s%ny>2) then
         do j=2,s%ny
            do i=2,s%nx
               ...
               l    = 1.d0/s%dsdnzi(i,j)
               ! Robert: probably need something like nuh = max(par%nuh,par%nuhfac*s%hh(i,j)*(s%DR(i,j)/par%rho)**(1.0d0/3.0d0))
               ! Robert: this doesn't seem to be consistent with Smag approach
               !nuhlocal = max(par%nuh,par%nuhfac*s%hh(i,j)*(s%DR(i,j)/par%rho)**(1.0d0/3.0d0))
               !s%nuh(i,j) = nuhlocal**2 * l * Tau * real(s%wetu(i,j)*s%wetu(i-1,j)*s%wetv(i,j)*s%wetv(i,j-1),kind=8)
               ! s%nuh(i,j) = nuh**2 * l * Tau * real(s%wetu(i,j)*s%wetu(i-1,j)*s%wetv(i,j)*s%wetv(i,j-1),kind=8)
         ...
         do i=2,s%nx
            dudx = (abs(s%uu(i,j)-s%uu(i-1,j))+par%nuhfac*(s%DR(i,j)/par%rho)**(1.0d0/3.0d0))/s%dsz(i,j)
            ...
            ! Robert: probably need something like nuh = max(par%nuh,par%nuhfac*s%hh(i,j)*(s%DR(i,j)/par%rho)**(1.0d0/3.0d0))
            !nuhlocal = max(par%nuh,par%nuhfac*s%hh(i,j)*(s%DR(i,j)/par%rho)**(1.0d0/3.0d0))
            !s%nuh(i,j) = nuhlocal**2 * l * Tau * real(s%wetu(i,j)*s%wetu(i-1,j),kind=8)
