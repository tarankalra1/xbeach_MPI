...
contains
   ...
   subroutine ship_init(s,par,sh)
      ...
      use xmpi_module
      use spaceparams
      use readkey_module, only: count_lines, readkey_dbl, readkey_int, readkey_intvec, readkey_name, read_v
      ...
      if(par%ships==1) then
         ...
         do i=1,par%nship
            ...
            sh(i)%read_heading   = readkey_int(sh(i)%name,'read_heading',  0,  0, 1)
            ...
            if (sh(i)%flying==0) then
               if (sh(i)%read_heading==0) then
                  do it=1,sh(i)%track_nt
                     call read_v(fid,sh(i)%track_t(it),sh(i)%track_x(it),sh(i)%track_y(it))
                  enddo
               else
                  do it=1,sh(i)%track_nt
                     ! also read in track_dir, in nautical deg
                     call read_v(fid,sh(i)%track_t(it),sh(i)%track_x(it),sh(i)%track_y(it),sh(i)%track_dir(it))
                     ! convert to cartesian direction
                     sh(i)%track_dir(it)=par%px/180d0*(270d0-sh(i)%track_dir(it))
                  enddo
               endif
               sh(i)%track_z=0.d0
            else
               if (sh(i)%read_heading==0) then
                  ...
                  do it=1,sh(i)%track_nt
                     ! also read in track_dir, in nautical deg
                     call read_v(fid,sh(i)%track_t(it),sh(i)%track_x(it),sh(i)%track_y(it),sh(i)%track_dir(it),sh(i)%track_z(it))
                     ! convert to cartesian direction
                     sh(i)%track_dir(it)=par%px/180d0*(270d0-sh(i)%track_dir(it))
                  enddo
               endif
   ...
   subroutine shipwave(s,par,sh)
      ...
      use readkey_module
      use filefunctions
      use interp,         only: grmap, grmap2, mkmap, linear_interp
