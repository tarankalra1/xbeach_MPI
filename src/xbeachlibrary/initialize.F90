...
contains
   ...
   subroutine flow_init (s,par)
      ...
      allocate(s%qmag(1:s%nx+1,1:s%ny+1))
      ...
      allocate(s%zswet(1:s%nx+1,1:s%ny+1))
      ...
      ! s%zs0 = max(s%zs0,s%zb)  ! RM: why is this statement here?
      ! cjaap: replaced par%hmin by par%eps
      if (par%hotstart .ne. 1) s%zs=max(s%zb+epsilon(0.d0),s%zs0)
      ...
      s%tauby=0.d0
      s%maxzs= dFill
      s%minzs= dFill ! RM: use negative initialization here too, needed for NaN output mask. Computation in flow_timestep.F90
      s%zswet= dFill ! RM: use to keep track of max/min water levels of wet points (no bed levels)
