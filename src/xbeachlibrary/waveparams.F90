...
contains
   ...
   subroutine makebcf(par,s,wp)
      ...
      use readkey_module, only: readkey_dbl
   ...
   subroutine build_jonswap(par,s,wp,fname)
      ...
      use readkey_module, only: readkey_dbl, readkey
      use params,         only: parameters
      use spaceparamsdef
      use xmpi_module
   ...
   subroutine swanreader(par,s,wp,fname)
      ...
      use math_tools
      use logging_module, only: writelog
      use math_tools,     only: flipa, flipv
   ...
   subroutine build_etdir(par,s,wp,Ebcfname)
      ...
      use params,         only: parameters
      use math_tools,     only: fftkind, random, fft, flipiv, hilbert
      use spaceparams
      ...
      do i=1,wp%K
         wp%CompFn(1,wp%index_vector(i)) = A(i)/2*exp(compi*phase(i))           ! Bas: wp%index_vector used in time dimension because t=j*dt in frequency space
      enddo
      ...
      do index2=2,wp%Npy
         ...
         exp(-compi*k*(dsin(wp%theta0)*(s%yz(1,index2)-s%yz(1,1)) &
   ...
   subroutine build_boundw(par,s,wp,qbcfname)
      ...
      do m=1,4
         ...
         do jj=2,Npy
            ...
            Ftemp2 = Ftemp(:,:,m)*exp(-1*compi*(KKy*(s%yz(1,jj)-s%yz(1,1))+KKx*(s%xz(1,jj)-s%xz(1,1))))
